...
function Chat({ chat, deleteChat, connectedUsers }) {
  ...
  return (
    <>
      <List selection>
        <List.Item
          active={router.query.messages === chat.messagesWith}
          onClick={() =>
            router.push(`/messages?message=${chat.messagesWith}`, undefined, {
              // undefined for hide the id in searcbar
              shallow: true, // updating the path of the current page without rerunning
            })
          }
        >
          <Comment>
